...
let API_DATA = {
    ...
    // Total families count from API (fallback to record count)
    totalFamilies: 0
};
...
// ================== MAIN FILTER API FUNCTION WITH PROPER AND/OR STRUCTURE ==================

async function loadFilteredData(filters) {
    try {
        console.log("Building filter conditions with AND/OR structure...");
        console.log("Current scheme logic:", schemeLogic);

        // Collect geographic filters for AND condition
        const andConditions = {};
        ...
        // Build filterConditions based on schemeLogic and available filters
        let filterConditions = {};

        if (schemeFilters.length === 0) {
            // No scheme filters, just use geographic filters if any
            ...
            // AND logic with scheme IDs - use comma-separated string in AND
            if (Object.keys(andConditions).length > 0) {
                // Combine geographic filters with scheme IDs in AND
                andConditions.type_id = schemeFilters.join(',');
                filterConditions = {
                    "and": andConditions
                };
            } else {
                // Only scheme IDs with AND
                filterConditions = {
                    "and": {
                        "type_id": schemeFilters.join(',')
                    }
                };
            }
        } else {
            // OR logic with scheme IDs
            if (Object.keys(andConditions).length > 0) {
                // For OR logic with geographic filters, we need a structure where:
                // (district AND taluk AND...) AND (type_id IN [scheme1, scheme2, ...])
                // But since your API seems to support comma-separated in OR, we'll use that
                ...
                console.log("Using structure: AND(geographic) with OR(schemes)");
            } else {
        ...
        if (response && response.success === 1) {
            ...
            if (Array.isArray(filteredList)) {
                console.log(`API returned ${filteredList.length} records`);
                // Keep full list for pie chart (no dedupe here); duplicates removed only for scheme descriptions
                API_DATA.filteredData = filteredList;

                // Use API total_count as total families (if provided)
                if (response && response.total_count != null && !isNaN(Number(response.total_count))) {
                    API_DATA.totalFamilies = Number(response.total_count);
                } else if (response && response.total_families != null && !isNaN(Number(response.total_families))) {
                    API_DATA.totalFamilies = Number(response.total_families);
                } else {
                    API_DATA.totalFamilies = filteredList.length;
                }
            ...
            if (response && response.message === 'No data found') {
                console.log("API returned 'No data found'");
                API_DATA.filteredData = [];
                return [];
            } else {
                // Fallback: return empty; no extra analytics API call
                API_DATA.filteredData = [];
                return [];
            }
        }
...
async function testAndOrApiStructure() {
    try {
        ...
        if (response && response.success === 1) {
            console.log("✅ AND/OR API structure accepted successfully!");
            const decryptedData = decryptData(response.data);
            console.log("Test response data:", decryptedData);
            return true;
        } else {
            console.log("❌ API rejected AND/OR structure");
            ...
            if (response2 && response2.success === 1) {
                console.log("✅ AND-only structure accepted successfully!");
                return 'and-only';
            }
...
// Update scheme descriptions from already-fetched filtered data (no extra API call)
function updateSchemeDescriptionsFromData(filteredData) {
    const descriptionsContainer = $('#schemeDescriptions');
    descriptionsContainer.empty();
    ...
    if (selectedSchemes.length === 0 || !Array.isArray(filteredData) || filteredData.length === 0) {
    ...
    // Remove duplicates by ufcno for scheme descriptions only (pie chart keeps full count)
                ...
                <div class="district-header d-flex justify-content-between align-items-center mb-2 p-2 bg-light rounded">
                    <div>
                        ...
                        <small class="text-muted">
                            ${districtEntries.length} entries • 
                            ${districtUniqueRemarksEntries.length} unique remarks
                            ${districtWithoutRemarks.length > 0 ? ` • ${districtWithoutRemarks.length} without remarks` : ''}
                        </small>
                    </div>
                </div>
...
function updateGeographicDistributionChart(filteredData) {
    ...
    const totalFamilies = API_DATA.totalFamilies || filteredData.length;

    // If schemes are selected -> show scheme-wise family counts
    const schemeSelected = [1, 2, 3].some(i => {
        const category = $(`#schemeCategory${i}`).val();
        const schemeId = $(`#schemeList${i}`).val();
        return category && category !== 'ALL' && schemeId && schemeId !== 'ALL';
    });
    ...
    if (schemeSelected) {
        const countsByScheme = new Map();

        filteredData.forEach((row, idx) => {
            const schemeId = row.type_id;
            if (!schemeId) return;

            const schemeName = row.scheme_name || `Scheme ${schemeId}`;
            if (!countsByScheme.has(schemeId)) {
                countsByScheme.set(schemeId, { name: schemeName, families: new Set() });
            }

            const entry = countsByScheme.get(schemeId);
            const famKey = row.ufcno != null ? `ufc_${row.ufcno}` : `row_${idx}`;
            entry.families.add(famKey);
        });

        displayData = Array.from(countsByScheme.values()).map(entry => ({
            name: entry.name,
            value: entry.families.size
        }));

        titleText = `Scheme Distribution (Total families: ${totalFamilies})`;
        seriesName = 'Scheme Distribution';
        filterSummaryText = `(${displayData.reduce((s, x) => s + x.value, 0)} scheme-family counts across ${displayData.length} scheme${displayData.length !== 1 ? 's' : ''})`;
    } else {
        // Group data by district (existing behaviour)
        ...
        titleText = `Geographic Distribution (Total: ${totalCount})`;
        seriesName = 'District Distribution';
        const districtCount = Object.keys(districtCounts).length;
        filterSummaryText = `(${totalCount} records across ${districtCount} district${districtCount !== 1 ? 's' : ''})`;
    }
    ...
    $('#filterSummary').text(filterSummaryText);
}

// Update summary cards
function updateSummaryCards(filteredData) {
    const totalCount = filteredData.length;
    const totalFamilies = API_DATA.totalFamilies || totalCount;
    ...
    // Update summary cards
    $('#totalFamiliesCount').text(totalFamilies.toLocaleString());
    $('#activeFamiliesCount').text(totalFamilies.toLocaleString());
...
async function applyFilters() {
    ...
    try {
        // Single API call for analytics_suggestionform_smple; use result for chart and descriptions
        const filteredData = await loadFilteredData(filters);

        // Update the chart with filtered data
        updateGeographicDistributionChart(filteredData);

        // Build scheme descriptions from same filtered data (no extra API call)
        updateSchemeDescriptionsFromData(filteredData);
